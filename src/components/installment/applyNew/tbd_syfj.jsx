...
class Insurance extends Component{
    ...
    getSyx(){
        ...
        for(var i in refArr){
            ...
            value.syfjx=value.syfjx*100;
            syxTotal.push(value)
        }
        return syxTotal;
    }
    syx(){
        var syx=0;console.log(refArr)
        for(var i in refArr){
            var value=refArr[i].props.form.getFieldsValue();
            syx+=parseFloat(value.syfjx===''?0:value.syfjx);
        }
        console.log(syx)
        return syx;
    ...
    render() {
        ...
        var detailArr=this.props.fjxList;
                    ...
                    if(i==0){
                        data=(
                            <div key={'key'+(detailArr[i].ordinal===undefined?detailArr[i]:detailArr[i].ordinal)}>
                                ...
                                <Syx onRef={that.onRef.bind(that)} value={1} type={2} syx={that.getCalc.bind(that)} tbd={detailArr[i]} orderId={that.state.orderIds}></Syx>
                            </div>
                        )
                    }else{
                        data=(
                            <Row key={'key'+(detailArr[i].ordinal===undefined?detailArr[i]:detailArr[i].ordinal)} id={'dels'+(detailArr[i].ordinal===undefined?detailArr[i]:detailArr[i].ordinal)}>
                                <div>
                                    ...
                                    <Syx onRef={that.onRef.bind(that)} value={num+1} type={2} syx={that.getCalc.bind(that)} tbd={detailArr[i]} orderId={that.state.orderIds}></Syx>
                                </div>
                            </Row>
                        )
                    }
                    ...
                    var data=(
                        <Row key={'keys'+detailArr[i]} id={'dels'+detailArr[i]}>
                            <div>
                                ...
                                <Syx onRef={that.onRef.bind(that)} value={delNum+2} type={2} syx={that.getCalc.bind(that)}></Syx>
                            </div>
                        </Row>
                    )
                    tbdList.push(data);
            ...
            <Form className='fqsq'>
                <Row>
                    <div className='genre'>
                        ...
                        <span className='genreTitle'>商业附属险</span>
                    </div>
                    <Row>
                        <FormItem
                        ...
                        {
                            type===0&&this.state.orderIds===undefined?<Syx onRef={this.onRef.bind(this)} value={1} type={2} syx={this.getCalc.bind(this)}></Syx>:''
                        }
                        {
